...
=======
>>>>>>> parent of 6e249a3... Update for webuzo 
...
function variable_initialisation() {
	...
	function match_pool_to_config_file() {
		for ((p = 0; p <= no_of_pools; p++)); do
			for ((i = 0; i <= no_of_includes; i++)); do
...
function display_pool_information() {
	...
	for ((i = 0; i <= no_of_pools; i++)); do
		...
		echo -en $(echo "scale =2; ${total_pool_mem_usage[$i]}" / 1024 | bc)
		echo -en " MB\e[0m\t"
		...
		echo -en $(echo "scale =2; ${pool_ave_process_size[$i]}" / 1024 | bc)
		echo -e " MB\e[0m"
...
function display_server_memory_information() {
	...
	echo -ne "\t\t\t\t\e[1m"
	echo -ne $(echo "scale =2; $total_server_memory" / 1024 | bc)
	echo -e " MB\e[0m"
	...
	echo -ne "\t\e[1m"
	echo -ne $(echo "scale =2; $total_phpfpm_allowed_memory" / 1024 | bc)
	echo -e " MB\e[0m"
	...
	echo -en "\t\t\e[1m"
	echo -ne $(echo "scale =2; $total_phpfpm_mem_usage_average_process" / 1024 | bc)
	echo -en " MB\e[0m "
...
function detailed_pool_recommendations() {
	...
	for ((i = 0; i <= no_of_pools; i++)); do
		...
		echo -ne $(echo "scale =2; ${total_pool_mem_usage[$i]}" / 1024 | bc)
		echo -ne " MB\e[0m"
		echo -ne "\tAllowed usage: \e[1m"
		echo -ne $(echo "scale =2; ${pool_allowed_mem_use[$i]}" / 1024 | bc)
		echo -ne " MB\e[0m"
		echo
	done
	echo
